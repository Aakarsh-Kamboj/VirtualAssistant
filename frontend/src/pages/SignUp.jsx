...
function SignUp() {
  ...
  return (
    <div className='w-full h-screen bg-cover flex justify-center items-center' style={{backgroundImage:`url(${bg})`}}>
      <form className='w-[90%] h-150 max-w-125 bg-[#e3d0d00a] backdrop-blur-md shadow-lg shadow-blue-950 flex flex-col items-center justify-center gap-5 px-5' onSubmit={handleSignUp}>
        <h1 className='text-white text-[30px] font-semibold mb-7.5'>Register to <span className='text-blue-400'>Virtual Assisant</span></h1>
        <input type="text" placeholder='Enter your Name' className='w-full h-15 outline-none border-2 border-white bg-transparent text-white placeholder-gray-300 px-5 py-2.5 rounded-full text-[18px]' onChange={(e)=>setName(e.target.value)} value={name} />
        ...
        <button type='submit' className='min-w-37.5 h-15 mt-7.5 bg-white hover:bg-blue-700 text-black font-semibold rounded-full text-[20px] cursor-pointer' disabled={loading}>{loading?"Loading..":"Sign Up"}</button>
